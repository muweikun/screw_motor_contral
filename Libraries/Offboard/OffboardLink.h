	...
	class Subscriber
	{
		public:
			...
			void processByte(uint8_t data)
			{
				...
				if(index >= 3 && (rec_buffer[2] <= OFFBOARDLINK_MSG_MAX_NUM && sub_list[rec_buffer[2]] == nullptr)) 
				{
					index = 0;
					return;
				}
				
				if(index >= 4 && rec_buffer[3] != sub_list[rec_buffer[2]]->data_len) 
				{
					index = 0;
					return;
				}
				if(index == rec_buffer[3] + 6) 
				{
