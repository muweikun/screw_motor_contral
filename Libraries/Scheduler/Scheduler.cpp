...
void Scheduler::run(void)
{
  for(uint16_t i = 0; i < async_num; i ++)
  {
    ...
    {
			timeus_t start_t = micros();
      (*asyncs[i])();
			used_duration += micros() - start_t;
      //asyncs[i]->run = true;
    }
  }
  ...
  for(uint16_t i = 0; i < task_num; i ++)
  {
    ...
    if((delta_time_us >= tasks[i]->interval_tick_us) && (tasks[i]->interval_tick_us))
    {
      ...
      else
      {
        ...
        tasks[i]->init();
			...
			used_duration += micros() - tasks[i]->last_tick_us;
      //判断任务是否需要注销
	...
	if(micros() - last_usage_cal_t > full_duration || micros() < last_usage_cal_t - full_duration) 
	{
		last_usage_cal_t = micros();
		usage = (float)used_duration / full_duration;
		used_duration = 0;
	}
}
}