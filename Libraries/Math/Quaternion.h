/**
  ...
  ******************************************************************************
  */
#ifndef __ROBO_LIB_QUATERNION_H__
#define __ROBO_LIB_QUATERNION_H__
...
struct QuaternionT // 模板结构体
{
  ...
  {
    return robo_lib::isZero(w) && robo_lib::isZero(x) && robo_lib::isZero(y) && robo_lib::isZero(z);
  }
  ...
  {
    T recip_norm = (T)robo_lib::invSqrt(w * w + x * x + y * y + z * z);
    w *= recip_norm;
    x *= recip_norm;
    y *= recip_norm;
    z *= recip_norm;
  ...
  robo_lib::Matrix3<T> toRotationMatrix(void) const // 四元数转旋转矩阵
  {
	...
	QuaternionT<T> fromRotationMatrix(robo_lib::Matrix3<T> dst)
	{
		QuaternionT<T> q;
		q.w = (1.0 / invSqrt(1 + dst.i.x + dst.j.y + dst.k.z)) / 2.0;
		q.x = (dst.k.y - dst.j.z) / (4 * q.w);
		q.y = (dst.i.z - dst.k.x) / (4 * q.w);
		q.z = (dst.j.x - dst.i.y) / (4 * q.w);
