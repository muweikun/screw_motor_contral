/**
  ...
  ******************************************************************************
  */
#ifndef __ROBO_LIB_MATRIX3_H__
#define __ROBO_LIB_MATRIX3_H__
...
{
public:
  robo_lib::Vector3<T> i, j, k; // i,j,k为三维矩阵的三个列向量
  ...
  // 通过向量构造三维矩阵
  Matrix3<T>(const robo_lib::Vector3<T> i0, const robo_lib::Vector3<T> j0, const robo_lib::Vector3<T> k0)
  {
    i = i0;
    j = j0;
    k = k0;
  ...
  // 三维矩阵围绕轴旋转
  Matrix3<T> rotateAboutAxis(robo_lib::Vector3<T> axis, T angle) const
  {
    Matrix3<T> dst;
    ...
    robo_lib::Vector3<T> cross = i.crossProduct(axis);
  ...
  // 获取行向量，'x','y','z'
  robo_lib::Vector3<T> getRowVector(uint8_t row)
  {
    robo_lib::Vector3<T> dst;
    switch(row)
    {
    case 'x':
      dst.x = i.x;
  ...
  // 三维矩阵右乘向量，转换为向量
  robo_lib::Vector3<T> translateVector(const robo_lib::Vector3<T> & vec) const
  {
    robo_lib::Vector3<T> dst;
